    ...
    var shopApp = function () {
        ...
        _createClass(shopApp, [{
            ...
            value: function beforeRegister() {
                ...
                this.observers = ['_routePageChanged(routeData.page)', '_appChanged(subroute)', '_computeMenu(app,path)'];
            }
            ...
            key: '_computeMenu',
            value: function _computeMenu(app, path) {

                console.log(app, path);

                app.database().ref(path + '/sidebar/items').on('child_added', function (snap) {
                    var item = snap.val();
                    var key = snap.key;
                    var menuItems = [];
                    ...
                    app.database().ref(path + '/sidebar/items/' + key + '/items').on('child_added', function (snap2) {
                ...
                app.database().ref(path + '/categories/byId').on('child_added', function (snap) {
                    var item = snap.val();
                    var key = snap.key;
                    item.items = null;
                    console.log(item);
            ...
            key: 'handleResponse',
            value: function handleResponse(_, e) {
